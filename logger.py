# logger.py
# Commit: глушим мусор aiohttp.server; фильтр и для консоли; расширен список сигнатур; мелкие правки
...
from aiohttp.http_exceptions import (
    HttpProcessingError,
    BadHttpMessage,
    BadStatusLine as AioBadStatusLine,
)
import config
...
class IgnoreBadStatusLineFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if any(sub in msg for sub in self.NOISE_SNIPPETS):
            return False
        return True
...
class TelegramHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        if record.exc_info:
            ...
            if etype and issubclass(
                etype, (http.client.BadStatusLine, httpx.RemoteProtocolError, HttpProcessingError, BadHttpMessage, AioBadStatusLine)
            ):
                return
...
def configure_logging():
    ...
    # Глобально режем сетевой мусор (затронет все потомки)
    root.addFilter(IgnoreBadStatusLineFilter())

    # Консоль: логический INFO
    ch = logging.StreamHandler(sys.stdout)
    ch.setLevel(logging.INFO)
    ch.setFormatter(formatter)
    ch.addFilter(IgnoreBadStatusLineFilter())   # ← теперь и на консоли
    ...
    # Точно глушим исключения от aiohttp.server (логгер web_protocol/log_exception)
    aiohttp_srv = logging.getLogger("aiohttp.server")
    aiohttp_srv.setLevel(logging.ERROR)
    aiohttp_srv.addFilter(IgnoreBadStatusLineFilter())

    # uvicorn
    access = logging.getLogger("uvicorn.access")
    access.setLevel(logging.INFO)
    access.addFilter(IgnoreStaticPathsFilter(["/favicon.ico", "/robots.txt", "/sitemap.xml", "/config.json", "/.env", "/.git/"]))

    uvicorn_err = logging.getLogger("uvicorn.error")
    uvicorn_err.addFilter(IgnoreBadStatusLineFilter())
