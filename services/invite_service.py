...
async def generate_invite_links(bot, user, uid, PRIVATE_DESTINATIONS, verify_user, ERROR_LOG_CHANNEL_ID):
    ...
    for dest in PRIVATE_DESTINATIONS:
        if not all(k in dest for k in ("title", "chat_id", "description")):
            logging.error(
                f"[{func_name}] – user_id={uid} – Некорректный PRIVATE_DESTINATIONS: {dest}",
                extra={"user_id": uid}
            )
            continue
        ...
        try:
            ...
            try:
                ...
                logging.error(
                    f"[{func_name}] – user_id={uid} – Ошибка при получении ссылок из БД: {e}",
                    extra={"user_id": uid}
                )
                continue
            ...
            if existing and existing.get("expires_at"):
                try:
                    ...
                    if exp > now:
                        ...
                        logging.info(
                            f"[{func_name}] – user_id={uid} – Ссылка для «{title}» актуальна, используем её",
                            extra={"user_id": uid}
                        )
                        continue
                    else:
                        logging.info(
                            f"[{func_name}] – user_id={uid} – Ссылка для «{title}» устарела, генерируем новую",
                            extra={"user_id": uid}
                        )
                except Exception as e:
                    logging.warning(
                        f"[{func_name}] – user_id={uid} – Ошибка при проверке срока ссылки «{title}»: {e}",
                        extra={"user_id": uid}
                    )
            ...
            if isinstance(chat_id, int):
                try:
                    ...
                    try:
                        ...
                        logging.info(
                            f"[{func_name}] – user_id={uid} – Сгенерирована и сохранена ссылка для «{title}»: {invite_link} (результат: {save_result})",
                            extra={"user_id": uid}
                        )
                    except Exception as e:
                        logging.error(
                            f"[{func_name}] – user_id={uid} – Ошибка при сохранении ссылки в БД для «{title}»: {e}",
                            extra={"user_id": uid}
                        )
                except TelegramBadRequest as e:
                    text_err = str(e).lower()
                    logging.error(
                        f"[{func_name}] – user_id={uid} – TelegramBadRequest для «{title}»: {e}",
                        extra={"user_id": uid}
                    )
                    if any(kw in text_err for kw in ("limit", "too many requests", "invite links limit")):
                        try:
                            ...
                            if old_links:
                                ...
                                logging.info(
                                    f"[{func_name}] – user_id={uid} – Отозвана старая ссылка {oldest} в чате {chat_id}",
                                    extra={"user_id": uid}
                                )
                                try:
                                    ...
                                    logging.warning(
                                        f"[{func_name}] – user_id={uid} – Не удалось уведомить об отзыве ссылки {chat_id}: {ee}",
                                        extra={"user_id": uid}
                                    )
                                invite = await bot.create_chat_invite_link(
                                    chat_id=chat_id,
                                ...
                                buttons.append([{"text": title, "url": invite_link}])
                                logging.info(
                                    f"[{func_name}] – user_id={uid} – Сгенерирована новая ссылка после отзыва: {invite_link}",
                                    extra={"user_id": uid}
                                )
                        except Exception as e2:
                            logging.warning(
                                f"[{func_name}] – user_id={uid} – Ошибка при отзыве/повторной генерации ссылки: {e2}",
                                extra={"user_id": uid}
                            )
                except Exception as e:
                    logging.error(
                        f"[{func_name}] – user_id={uid} – Ошибка при создании новой invite-ссылки для {chat_id}: {e}",
                        extra={"user_id": uid}
                    )
            ...
            # 4) Если chat_id — URL, используем напрямую
                ...
                logging.info(
                    f"[{func_name}] – user_id={uid} – Используем прямую ссылку для «{title}»: {chat_id}",
                    extra={"user_id": uid}
                )

        except Exception as e:
            logging.error(
                f"[{func_name}] – user_id={uid} – Непредвиденная ошибка при обработке «{title}»: {e}",
                extra={"user_id": uid}
            )
            try:
                await bot.send_message(
                ...
                logging.warning(
                    f"[{func_name}] – user_id={uid} – Не удалось отправить уведомление об ошибке invite: {ee}",
                    extra={"user_id": uid}
                )
