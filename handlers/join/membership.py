...
async def add_user_and_membership(user, chat_id: int = None):
    ...
    try:
        ...
        logging.info(
            f"user_id={user.id} – Пользователь добавлен в БД username={user.username}, full_name={user.full_name}",
            extra={"user_id": user.id}
        )
    except Exception as exc:
        logging.error(
            f"user_id={user.id} – Ошибка add_user: {exc}",
            extra={"user_id": user.id}
        )
    ...
    if not chat_id:
        logging.warning(
            f"user_id={user.id} – chat_id не передан, используем BOT_ID={config.BOT_ID}",
            extra={"user_id": user.id}
        )
    ...
    if not chat_id:
        logging.error(
            f"user_id={user.id} – Нет chat_id, пропускаем add_membership",
            extra={"user_id": user.id}
        )
        return
    ...
    try:
        ...
        logging.info(
            f"user_id={user.id} – Подписан на chat={chat_id}",
            extra={"user_id": user.id}
        )
    except HTTPStatusError as exc:
        logging.warning(
            f"user_id={user.id} – HTTP {exc.response.status_code} при add_membership(chat={chat_id})",
            extra={"user_id": user.id}
        )
        await log_and_report(exc, f"{func_name} add_membership(user={user.id}, chat={chat_id})")
    except Exception as exc:
        logging.error(
            f"user_id={user.id} – Ошибка add_membership(chat={chat_id}): {exc}",
            extra={"user_id": user.id}
        )
        await log_and_report(exc, f"{func_name} add_membership(user={user.id}, chat={chat_id})")
...
async def on_startup():
    ...
    try:
        ...
        logging.info(
            f"user_id=system – Загружены tracked_chats: {tracked_chats}",
            extra={"user_id": "system"}
        )
    except Exception as exc:
        logging.error(
            f"user_id=system – Ошибка при загрузке tracked_chats: {exc}",
            extra={"user_id": "system"}
        )
    ...
    try:
        ...
        logging.error(
            f"user_id={config.BOT_ID} – Ошибка upsert_chat(bot): {exc}",
            extra={"user_id": config.BOT_ID}
        )
        await log_and_report(exc, f"upsert_chat(bot)")
...
async def on_chat_member(update: ChatMemberUpdated):
    ...
    logging.info(
        f"user_id={user_id} – Получен status='{status}' для chat={chat_id}",
    ...
    try:
        ...
        logging.error(
            f"user_id={user_id} – Ошибка add_user: {exc}",
            extra={"user_id": user_id}
        )
    ...
    if status == "member":
        try:
            ...
            logging.info(
                f"user_id={user_id} – Присоединился к chat={chat_id}",
                extra={"user_id": user_id}
            )
        except Exception as exc:
            logging.error(
                f"user_id={user_id} – Ошибка add_membership: {exc}",
                extra={"user_id": user_id}
            )
        ...
        try:
            ...
            logging.info(
                f"user_id={user_id} – Отписался от chat={chat_id}",
                extra={"user_id": user_id}
            )
        except Exception as exc:
            logging.error(
                f"user_id={user_id} – Ошибка remove_membership: {exc}",
                extra={"user_id": user_id}
            )
...
async def on_my_chat_member(update: ChatMemberUpdated):
    ...
    logging.info(
        f"user_id={config.BOT_ID} – bot status '{status}' in chat={chat_id}",
        extra={"user_id": config.BOT_ID}
    )
    if status == "member":
        try:
            ...
            logging.info(
                f"user_id={config.BOT_ID} – Бот подписан на chat={chat_id}",
                extra={"user_id": config.BOT_ID}
            )
        except Exception as exc:
            logging.error(
                f"user_id={config.BOT_ID} – Ошибка bot add_membership: {exc}",
                extra={"user_id": config.BOT_ID}
            )
        ...
        try:
            ...
            logging.info(
                f"user_id={config.BOT_ID} – Бот отписался от chat={chat_id}",
                extra={"user_id": config.BOT_ID}
            )
        except Exception as exc:
            logging.error(
                f"user_id={config.BOT_ID} – Ошибка bot remove_membership: {exc}",
                extra={"user_id": config.BOT_ID}
            )
